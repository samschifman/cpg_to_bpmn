...
class Graph:
    ...
    def wipe_label(self, label):
        return self.wipe(f'''
            MATCH (n:{label}) DETACH DELETE n
        ''')

    def wipe(self, cypher):
        node_count, relationship_count = self.database_metrics()
        delete_result, runtime = self.query(cypher)
        node_count_after, relationship_count_after = self.database_metrics()
        return f'Deleted {node_count - node_count_after} nodes and {relationship_count - relationship_count_after} relationships in {runtime} seconds'
